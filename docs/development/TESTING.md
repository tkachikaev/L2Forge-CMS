...
Документ описывает правила подготовки данных для PHPUnit-тестов KaevCMS.
...
Сохранение, проверка подключения и удаление серверов выполняются через `LoginServerAdministration` и `GameServerAdministration`. Тесты интерфейса проверяют Livewire-поведение, а `ServerAdministrationTest` закрепляет общие инварианты прикладного слоя:
...
## Навигация и клиентский JavaScript

PHPUnit проверяет серверную разметку и регистрацию клиентских инициализаторов. Реальное повторное выполнение редактора, QR-кода 2FA, переходов назад/вперёд и сохранённой боковой панели проверяется Playwright:
